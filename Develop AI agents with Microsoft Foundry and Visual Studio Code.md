...
### Sample answers
1. Portal is low-code and quick for prototyping; VS Code is better for version control, collaboration, and complex workflows.
2. Use custom tools when the agent needs real data, actions, or integration with APIs.
3. Grounding data provides factual context so the agent can answer based on real documents.
4. Store secrets in Key Vault or environment variables and avoid hard-coding them.
5. `response.completed` indicates the workflow or response is finished and final output is available.
